...
async def update_telegram_ui(message_id: int, text: str, reply_markup: dict = None):
    ...
    payload = {"chat_id": STAFF_CHAT_ID, "message_id": message_id, "text": text, "parse_mode": "HTML"}
...
async def get_dashboard_stats(db: AsyncSession = Depends(get_db)):
    """Provides the live data feed for the React Executive Dashboard."""
    ...
    staff_stats = {}
    for a in alerts:
        if a.claimed_by: staff_stats[a.claimed_by] = staff_stats.get(a.claimed_by, 0) + 1
    
    return {
        "totalAlerts": len(alerts),
        "avgResponseTime": round(avg_res, 1),
        "resolvedCount": len([a for a in alerts if a.status == "RESOLVED"]),
        "alerts": [{"room": a.room_number, "guest": a.guest_name, "issue": a.issue, "status": a.status, "created_at": a.created_at.isoformat()} for a in alerts],
        "topResponders": [{"name": k, "claims": v} for k, v in sorted(staff_stats.items(), key=lambda x: x[1], reverse=True)[:5]]
    }
...
async def telegram_callback(update: dict, db: AsyncSession = Depends(get_db)):
    ...
    if callback_data.startswith("ack_"):
        ...
        if task:
            ...
            await update_telegram_ui(message_id, f"🚧 <b>In Progress: Room {room}</b>\nClaimed by: {user}", {"inline_keyboard": [[{"text": "🏁 Mark Resolved", "callback_data": f"res_{room}"}]]})
...
async def trigger_escalation(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        await client.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage", json={"chat_id": STAFF_CHAT_ID, "text": msg, "parse_mode": "HTML", "reply_markup": {"inline_keyboard": [[{"text": "✅ Claim Task", "callback_data": f"ack_{room}"}]]}})
