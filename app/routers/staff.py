...
async def trigger_escalation(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    # 1. Database Entry (Async) - with robust error handling
    try:
        logger.info(f"💾 Attempting to save escalation for Room {room}")
        ...
        try:
            ...
            pass  # Rollback might also fail if connection is broken
