...
# Prevents tests from expiring by ensuring dates are always relative to 'now'
TODAY = date.today()
...
async def test_check_rates_integration_not_found(test_client):
    """Integration test: 404 response for non-existent dates."""
    # Using a date far in the future unlikely to be in any seed data
    ...
    # MATCHED: Aligned with the exact error string in app/main.py
    assert "No rates found for this date." in response.json()["detail"]


@pytest.mark.asyncio
async def test_check_rates_integration_past_date(test_client):
    """Integration test: 400 validation error for past check-in dates."""
    response: Response = await test_client.post(
        "/check-rates",
...
async def test_check_rates_integration_multiple_rates(test_client, db_session):
    """Integration test: Ensuring the API selects the correct date record."""
    rate1 = Rate(
